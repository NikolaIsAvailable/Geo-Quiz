...
const MultipleChoice: React.FC<Props> = ({ quizChoices }) => {  
    ...
    const [goodAnswer, setGoodAnswer] = useState<number | string>('')
    const [quizChoicesNew, setQuizChoicesNew] = useState<QuizArray[]>([])
    ...
    useEffect(() => {
        ...
        const animateChoice = () => {
            setTimeout(() => {
                element0?.classList.add('fade-in')
            }
            ,250)
            setTimeout(() => {
            ...
            setTimeout(() => {
                element0?.classList.remove('fade-in')
                element0?.classList.add('opacity')
            }
            ,1100)
    
            setTimeout(() => {
                element1?.classList.remove('fade-in')
                element1?.classList.add('opacity')
            }
            ,1100)
            setTimeout(() => {
                element2?.classList.remove('fade-in')
                element2?.classList.add('opacity')
            }
            ,1100)
    
            setTimeout(() => {
                element3?.classList.remove('fade-in')
                element3?.classList.add('opacity')
            }
            ,1300)
        }
        animateChoice()

        const answer = quizChoices.filter((choice) => choice.answer === 'good')

        setGoodAnswer(answer[0].depart.num_dep)

        
    }, [quizChoices])

    const selectAnswer = (choice: QuizArray, index: number) => {
        setDisabledButton(false)
        const nextAniamtion = document.querySelector('.next-quiz')
        nextAniamtion?.classList.remove('next-pulse')
        const goodAnswer = (choice: QuizArray) => choice.answer === 'good'

        const map = document.querySelector('.france')
        map?.classList.add('grow-france')

        if (choice.answer === 'good') {
            audioGood.play()

            const goodChoice = document.getElementById(`${index}`)
            const departMap = document.getElementById(`${choice.depart.num_dep}`)
            
            goodChoice?.classList.add('good-answer')
            departMap?.setAttribute("fill", "green")
            departMap?.setAttribute("filter", "url(#dropshadowplus)")
        } else {
            audioBad.play()
            const badChoice = document.getElementById(`${index}`)
            const goodIndex = (newQuiz ? quizChoicesNew : quizChoices).findIndex(goodAnswer)
            const goodChoice = document.getElementById(`${goodIndex}`)
            const badDepartMap = document.getElementById(`${choice.depart.num_dep}`)
            const goodDepartMap = document.getElementById(`${(newQuiz ? quizChoicesNew : quizChoices)[goodIndex].depart.num_dep}`)
            
            console.log(goodIndex);
            
            badChoice?.classList.add('bad-answer')
            goodChoice?.classList.add('good-answer-show')
            badDepartMap?.setAttribute("fill", "red")
            badDepartMap?.setAttribute("filter", "url(#dropshadowplus)")
            goodDepartMap?.setAttribute("filter", "url(#dropshadowplus)")
            goodDepartMap?.setAttribute("fill", "green")
        } 
    }
    ...
    const nextQuiz = () => {
        ...
        nextAniamtion?.classList.add('next-pulse')
        element0?.classList.remove('opacity')
        element1?.classList.remove('opacity')
        element2?.classList.remove('opacity')
        element3?.classList.remove('opacity')

        const resetDisplay = () => {
            const choices = document.querySelectorAll('.choice')
            const departementsMap = document.querySelectorAll('path')

            departementsMap.forEach((departement) => {
                departement.removeAttribute("fill")
                departement.removeAttribute("filter")
                departement.removeAttribute("fill")
                departement.removeAttribute("filter")
                departement.removeAttribute("filter")
                departement.removeAttribute("fill")
            ...
            choices.forEach((choice) => {
                choice.classList.remove('good-answer')
                choice.classList.remove('bad-answer')
                choice.classList.remove('good-answer-show')
            })
        }
        resetDisplay()
        setNewQuiz(true)
        ...
        const answer = quizArray.filter((choice) => choice.answer === 'good')
        ...
        setQuizChoicesNew(quizArray)
        setDisabledButton(true)
    }
    
    return (
        <div className="container multiple-choice">
            <h2 className="fade-in">Quel est le Département n°{goodAnswer} ?</h2> 
            <button disabled={disabledButton} 
                    className="bouton fade-in next-quiz-button" 
                    onClick={nextQuiz}>Next {'>>'} 
                <span className="next-quiz"></span>
            </button>
            <div className="row choices">
                {(newQuiz ? quizChoicesNew : quizChoices).map((choice, index) => (
                    <div className="col-md-6 choice mx-auto mt-3" key={index} id={`${index}`}>
                        <div className="row justify-content-between align-items-center" style={{ height: '100%'}}>
                            <p>{choice.depart.dep_name}</p>
                            <button onClick={() => selectAnswer(choice, index)}
                                    className="bouton rounded-circle">
            ...
            <div className="france mx-auto mt-5">
